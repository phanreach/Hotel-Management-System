...
export default function LoginPageContent() {
  ...
  return (
    <div className="flex min-h-screen w-full flex-row">
      ...
      <div className="hidden lg:flex w-1/2 relative flex-col justify-end p-12 bg-cover bg-center" 
        ...
        <div className="relative z-10 text-white max-w-lg">
          ...
          <h1 className="text-4xl font-extrabold mb-4">Experience luxury like never before.</h1>
          <p className="text-lg text-white/80">Join our exclusive community to manage bookings and unlock member rates.</p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex flex-col items-center justify-center p-6 bg-white dark:bg-slate-900">
        <div className="w-full max-w-[440px] flex flex-col gap-8">
          <header>
            ...
            <p className="text-slate-500">Manage your bookings and explore exclusive offers</p>
          </header>
            ...
            {['signin', 'signup'].map((tab) => (
              <button
                ...
                className={`flex-1 py-2.5 rounded-lg text-sm font-medium transition-all ${
                  mode === tab ? 'bg-white dark:bg-slate-700 shadow-sm text-black dark:text-white' : 'text-slate-500'
                }`}
              >
          ...
          <form onSubmit={handleFormSubmit} className="flex flex-col gap-5">
            ...
            >
              {isLoading ? <Loader2 className="animate-spin w-5 h-5" /> : mode === 'signin' ? 'Sign In' : 'Sign Up'}
            </button>
          </form>
        </div>
      </div>
